...
SELECT
        ...
        CASE
            WHEN ankieta.adSeen = 'TAK' then 1
            else 0
        END as [Czy_Widziala_Reklame],
        [Kwota_Rachunku] = wizyta.Kwota,
        CASE
            WHEN ankieta.adIsCause = 'TAK' then 1
            else 0
    ...
    JOIN dbo.Junk as junk on 
        junk.Czy_widzial_reklame = [Czy_Widziala_Reklame]
        and junk.Czy_przyszla_z_powodu_reklamy = [Czy_Przyszla_Z_Powodu_Reklamy] 
        and [Czy_Zarezerwowal_Kolejna] = junk.Czy_zarezerwowal_kolejna 
        and (
            CASE
                WHEN wizyta.Kwota < 50 THEN 'Niska'
                WHEN wizyta.Kwota >= 50 and wizyta.Kwota < 150 THEN 'Średnia'
